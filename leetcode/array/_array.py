...
class BooyerMoore:
    ...
    combined
    """
    def majorityElement(self, nums: List[int]) -> int: # LC 169
        """
        TC: O(n) - single pass through array
        SC: O(1) - only two variables
        """
        candidate = None # Current majority candidate
        count = 0 # How many votes ahead the current candidate is
        
        for num in nums:
            if count == 0: # No current majority,
                candidate = num # Adopt this candidate as majority
                count = 1
            elif num == candidate: # Supporting vote for current candidate
                count += 1
            else: # Opposing vote, cancels out one candidate vote
                count -= 1
        
        return candidate

sol = BooyerMoore()
print("Majority Element:", sol.majorityElement([2,2,1,1,1,2,2]))  # 2
print("Majority Element:", sol.majorityElement([3,2,3]))  # 3
...
class MergeIntervals:
    ...
    def merge(self, intervals: List[List[int]]) -> List[List[int]]: # LC 56
        ...
        result = []
        prev_start, prev_end = intervals[0] # get initial start & end
