...
def problem(arr, target):
    ...
    for i in range(len(arr)): # Loop through array
        ...
        for j in range(i, len(arr)): # Loop thru ele's starting at j
            running_sum = running_sum + arr[j] # Calc new sum
...
def problem2(arr, target):
    left = 0 # Left pointer
    running_sum = 0 # Track current window sum
...
print(problem([1, -2, 7, 2, 3, -10] , 8)) # Answer = 6 (len 6 subarray)
print(problem2([1, -2, 7, 2, 3, -10] , 8)) # Answer = 4 which is incorrect, sliding window only works when all values are non negative