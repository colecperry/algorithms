...
# How to Solve - Utilize Floyd's cycle finding algorithm (Tortise and Hare)
    ...
    # Traverse the list -> Check that fast and fast.next are not null (takes care of even and odd length lists). If we are on the last node of the list and we go to fast.next.next we will get an error
    # Move slow ptr one step at a time, and fast ptr two
