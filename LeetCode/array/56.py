...
class Solution:
    def merge(self, intervals: List[List[int]]) -> List[List[int]]: # for loop
        ...
        for curr_start, curr_end in intervals[1:]: # iterate & unpack from i=1 
            if prev_end >= curr_start: # INTERVALS OVERLAP
                prev_end = max(prev_end, curr_end) # update end pointer -> use max() for edge case where interval is fully contained
        ...
        result.append([prev_start, prev_end]) # Append last interval
