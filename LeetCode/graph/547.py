...
# How to Solve (BFS):
    ...
    #   - Initialize a queue and add the city
    #   - While the queue is not empty:
    #       - Pop a city from the queue
    #       - Mark it as visited
    #       - Enqueue all unvisited neighbors connected to this city
    #   - After the queue is empty, we've finished exploring a province
    ...
    def findCircleNumBFS(self, isConnected: List[List[int]]) -> int:
        ...
        for city in range(n): # Loop through each city
            if not visited[city]: # If it's not visited
                ...
                while queue:
                    ...
                    for neighbor in range(n): # Loop through neighbors
                        if isConnected[current][neighbor] == 1 and not visited[neighbor]:
                            queue.append(neighbor) # If we find a new path, add it to explore
